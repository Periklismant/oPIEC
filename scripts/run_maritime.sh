...
fileName="maritime_short" # Name for the result file of Prob-EC/oPIEC.
...
inputPath="../../../datasets/examples/maritime_short_test/maritime_short.pl" # input path for Prob-EC. 
...
echo "Hello" &&
./fixoutput.sh ${repoPath} ${eventsString} ${valuesString} ${fileName} && # Seperate events from Prob-EC output. Results in the 'Prob-EC_output/recognition/' folder. 
cd ../src/oPIEC-python/ &&
LastIndex=$((${#events[@]}-1)) &&
for i in $(seq 0 ${LastIndex}) # Execute oPIEC for all target events.
do 
	echo "oPIEC for ${event[i]}" &&
	python${pythonVersion} -c 'import oPIEC; oPIEC.runoPIEC("../../", ${fileName}_${events[i]}_${values[i]})' # Adjust the parameters of oPIEC from /src/oPIEC-python/oPIEC.py
done